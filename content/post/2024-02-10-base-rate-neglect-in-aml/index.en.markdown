...
Let us assume both institutions use three different scenarios or models - Scenario A (Change in Behavior) ,Scenario B (Rapid Flow Through of Funds) , Scenario C (Large Reportable Transactions). 

I  will also assume the scenarios alert independent of each other.


I will assign the following sensitivity and specificity to these scenarios. 
...
|:----------|-----------:|-----------:|
|Scenario A |        0.81|        0.63|
|Scenario B |        0.81|        0.63|
|Scenario C |        0.81|        0.63|

I am assuming that bad actors have a 0.9 probability of exhibiting the underlying pattern being monitored  while regular citizens have a 0.7 probability of exhibiting the said pattern. For generic patterns like change in behavior or rapid movement of funds, this is a reasonable assumption.

Given  a customer exhibits this pattern, I am assuming there is a 0.9 probability that a scenario alerts on it. 

For each of the models,

Sensitivity is 0.81 (0.9 x 0.9). If the customer is truly a criminal, this is the probability that a model triggers and an alert.

Specificity is 0.37 (1 - 0.7 x 0.9). If the customer is a regular citizen, this is the probability that the model does not alert.
...
Even then the chances of the case being really suspicious is as low as 0.2%.
